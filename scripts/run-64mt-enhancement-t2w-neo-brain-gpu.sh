...
if [[ $# -ne 6 ]] ; then
    ...
    echo "bash /home/neonatal-low-field-mri-analysis/run-64mt-enhancement-t2w-neo-brain-gpu.sh [case_id] [full_path_to_input_64mt_.nii.gz] [full_path_to_folder_for_out_qc_and_metrics] [full_path_to_reoriented_output_enhanced_image.nii.gz] [full_path_to_reoriented_original_image.nii.gz] [full_path_to_output_label.nii.gz]"
    echo
    echo "------------------------------------------------------------"
    echo
    exit
...
#unset PYTHONPATH ;
python3 ${src}/src/run_monai_patch_atunet_segmentation_1case-2026-gpu.py 128 1 ${w_bet} ${proc}/pad-t2-128.nii.gz ${proc}/bet-lab-pad-t2-128.nii.gz
...
#unset PYTHONPATH ;
python3 ${src}/src/run_monai_patch_atunet_segmentation_1case-2026-gpu.py  128 2 ${w_lr} ${proc}/pad-masked-crop-tr-t2-128.nii.gz ${proc}/lr-lab-pad-masked-crop-t2-128.nii.gz
...
python3 ${src}/src/run_monai_basiunet_enhancement_1case-2026-gpu.py ${w_enhancement} ${proc}/pad-reo-masked-t2-128.nii.gz ${proc}/pad-reo-bet-128.nii.gz ${proc}/enhanced-reo-t2.nii.gz ${proc}/multi-lab-reo-t2.nii.gz ${proc}/qc-segmentation.nii.gz ${proc}/qc-prediction.nii.gz ${proc}/qc-contrast.nii.gz ${proc}/qc-summary.csv
...
cp ${proc}/r-d.dof ${out_folder}/${case_id}-from-org-to-rigid-reo.dof

${mirtk}/transform-image  ${proc}/qc-segmentation.nii.gz ${out_folder}/${case_id}-qc-segmentation.nii.gz -target  ${proc}/tmp-final-ref-masked.nii.gz  -dofin ${proc}/from-aff-to-rigid-reo.dof

${mirtk}/transform-image  ${proc}/qc-prediction.nii.gz ${out_folder}/${case_id}-qc-prediction.nii.gz -target  ${proc}/tmp-final-ref-masked.nii.gz  -dofin ${proc}/from-aff-to-rigid-reo.dof

${mirtk}/transform-image  ${proc}/qc-contrast.nii.gz ${out_folder}/${case_id}-qc-contrast.nii.gz -target  ${proc}/tmp-final-ref-masked.nii.gz  -dofin ${proc}/from-aff-to-rigid-reo.dof

${mirtk}/transform-image  ${proc}/org-t2.nii.gz ${out_org} -target  ${proc}/tmp-final-ref-masked.nii.gz  -dofin ${proc}/r-d.dof

cp ${proc}/qc-summary.csv ${out_folder}/${case_id}-qc-summary.csv

cp ${proc}/bet-lab-org-t2.nii.gz ${out_folder}/${case_id}-bet-lab-org-t2.nii.gz


echo
echo "------------------------------------------------------------"
echo
echo " - GENERATING STRUCTURAL METRICS ... "
echo
echo "------------------------------------------------------------"
echo
...
python3 ${src}/src/surface_volumetry_metrics_red_bounti_workbench.py -s ${out_lab}  --left_mesh ${out_folder}/${case_id}-left-hemisphere-white.surf.gii  --right_mesh ${out_folder}/${case_id}-right-hemisphere-white.surf.gii  --surface_csv ${out_folder}/${case_id}-surface-metrics.csv --volumes_csv ${out_folder}/${case_id}-volume-metrics.csv > ${proc}/tmp.txt
...
if [[ ! -f ${out_img} ]];then
    ...
    echo " - output processing folder with QC and structural measurements: " ${out_folder}
