...
if [[ $# -ne 6 ]] ; then
    ...
    echo "bash /home/neonatal-low-field-mri-analysis/run-64mt-enhancement-t2w-neo-brain-cpu.sh [case_id] [full_path_to_input_64mt_.nii.gz] [full_path_to_folder_for_out_qc_and_metrics] [full_path_to_reoriented_output_enhanced_image.nii.gz] [full_path_to_reoriented_original_image.nii.gz] [full_path_to_output_label.nii.gz]"
    echo
    echo "------------------------------------------------------------"
    echo
    exit
